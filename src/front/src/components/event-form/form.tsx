...
import { ActionIcon, Button, Container, Flex, Grid, Paper, Text, Textarea, TextInput, Title } from '@mantine/core'
import { useDebounce } from '@react-hook/debounce'
import { IconAlertTriangle, IconTrash } from '@tabler/icons-react'
import { UseMutationResult } from '@tanstack/react-query'
import React, { useEffect, useMemo } from 'react'
import { DefaultValues, FormProvider, SubmitHandler, useFieldArray, UseFieldArrayRemove, useForm, useFormContext } from 'react-hook-form'
import { PartialDeep } from 'type-fest'
import { z } from 'zod/v4'

import { AttendanceOptions, maybeGetAttendanceType } from '../../../../shared/attendance/attendance.js'
...
import { EventSchemaWhenCreating, TEventWhenCreating } from '../../../../shared/schemas/event.js'
 ...
 */
  ...
  const attendanceStructure = watch('attendance.attendanceStructure')
  const attendanceConfig = React.useMemo(() => maybeGetAttendanceType(attendanceStructure), [attendanceStructure])
      ...
      <Container>
        <Paper shadow="md" radius="md" withBorder mt={16} p="md">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {attendanceConfig ? <attendanceConfig.EventFormElement /> : null}
            {feeConfig ? <feeConfig.ConfigurationElement /> : null}
            <CustomQuestionsForm />
            <ValidationErrors />
            <Button variant="contained" type="submit" mt={16} disabled={!isValid} loading={mutation.isPending}>
              {mode == 'create' ? 'Create' : 'Edit'}
            </Button>
            {mode === 'edit' && (
              <Button ml={8} bg="red" variant="contained" type="button" mt={16} onClick={deleteFunc} loading={deleteEvent.isPending}>
                Delete
              </Button>
            )}
          </form>
