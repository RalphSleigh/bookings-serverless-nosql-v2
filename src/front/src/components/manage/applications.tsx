...
export const ManageApplications = () => {
  ...
  const approved = useMemo(
    () => applicationsQuery.data.applications.filter((a) => a.status === 'approved').sort((a, b) => (b.maxPredicted + b.minPredicted) / 2 - (a.maxPredicted + a.minPredicted) / 2),
  ...
  const totalMean = (totalApprovedMin + totalApprovedMax) / 2
