    ...
    if (discordDiffs.length > 0) {
      if (own) {
        await enqueueAsyncTask({
          type: 'discordMessage',
          data: {
            message: `${updatedBooking.data.basic!.name} (${updatedBooking.data.basic!.district}) edited their booking for event ${event.name}, they have booked ${people.length} ${people.length > 1 ? 'people' : 'person'} (previously ${existingForCompare.people.length})`,
          },
        })
      } else {
        await enqueueAsyncTask({
          type: 'discordMessage',
          data: {
            message: `${user.name} edited booking ${updatedBooking.data.basic!.name} (${updatedBooking.data.basic!.district}) for event ${event.name}, they have booked ${people.length} ${people.length > 1 ? 'people' : 'person'} (previously ${existingForCompare.people.length})`,
          },
        })
      }
