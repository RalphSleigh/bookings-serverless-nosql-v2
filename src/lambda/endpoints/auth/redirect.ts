...
export const authRedirect: RequestHandler = (req, res) => {
  ...
  const shouldSwitch = req.query.switch === 'true'
  const redirect_url = `https://${config.AUTH0_DOMAIN}/authorize?response_type=code&client_id=${config.AUTH0_CLIENT_ID}&redirect_uri=${config.BASE_URL}/api/auth/callback&scope=openid profile email&state=123${shouldSwitch ? '&prompt=login' : ''}`
